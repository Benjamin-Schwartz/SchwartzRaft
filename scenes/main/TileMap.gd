...
@onready var player = get_parent().get_node("raft")
...
func generate_chunk(pos):
	for x in range(width):
		for y in range(height):
			...
			var moist = moisture.get_noise_2d(pos.x - (width/2.0) + x, pos.y - (height/2.0) + y) * 10 # Values between -10 and 10
			var temp = temperature.get_noise_2d(pos.x - (width/2.0) + x, pos.y - (height/2.0) + y) * 10
			var alt = altitude.get_noise_2d(pos.x - (width/2.0) + x, pos.y - (height/2.0) + y) * 10
			...
			if alt < 5: # Arbitrary sea level value (choosing 0 will mean roughly 1/2 the world is ocean)
